	...
	<form name="reserveRight" id="reserveRight"
		...
		이 약관은 (주)그린카, 롯데렌탈(주) (이하 "임대인"/회사라 합니다)와 고객(이하 "임차인"이라 합니다.)
			사이의 대여용자동차 임대차계약(이하 "대여계약"이라 합니다.)상의 권리, 의무에 관한 사항을 규정함을 목적으로 합니다.
						...
						<label for="agreeUse"> <input type="checkbox"
							id="agreeUse" name="GC_Agree" value="" class="checkbox" />'가카
							차량이용'이용약관에 동의합니다.
						</label>
					</div>
					<!-- 이용약관 끝  -->
					<div id="containerLeft">
						...
						<ul class="list-group">
							<li class="list-group-item note-item clearfix"
								onClick="location.replace='/rentacar/car/reserve.do?car_no=${rentacarOne.car_no}'"
								...
								<div class="content-left panel-body pull-left">
									...
									<p>
									<h5>
										보험가입 확인 :&nbsp;<input type="radio" name="reserve_insure"
											...
											value="false" onclick="Rradio_OnOff('Radio_Off');" /> 비가입
										(보험가입시 별도의 비용이 발생할수 있습니다.)
									</h5>
								</div>
					...
					<div class="menu-wrap pull-left">
						<button type="button" onclick="onList()" class="btn btn-primary">취소</button>
					</div>
					<!-- 글목록 버튼 끝 -->
					<!-- 글쓰기 버튼 시작 -->
					<div class="menu-wrap pull-right">
						<%-- <c:if test="${session_email != null }"> --%>
							<!-- 로그인을 해야 글을 쓸수 있음 -->
							<button type="button" onclick="onWrite()" class="btn btn-primary">예약/결제</button>
						<%-- </c:if> --%>
					</div>
					<!-- 글쓰기 버튼 끝 -->
...
<input type="hidden" name="reserve_car_no" id="reserve_car_no"
	value="${rentacarOne.car_no}">
<input type="hidden" name="reserve_mem_no" id="reserve_mem_no"
	value="${session_num}">
<input type="hidden" name="reserve_sdate" id="reserve_sdate"
	value="${car_dt1}">
<input type="hidden" name="reserve_edate" id="reserve_edate"
	value="${car_dt2}">
<input type="hidden" name="reserve_slat" id="reserve_slat"
	value="${rentacarOne.car_lat}">
<input type="hidden" name="reserve_slng" id="reserve_slng"
	value="${rentacarOne.car_lng}">
